...
export const version = 1;
   ...
   * @internal
   */
export function factory (cs: any) {
     ...
     */
    function isPropertyAssignmentToTransform (node: any): node is PropertyAssignment {
        return ts.isPropertyAssignment(node) &&
        ts.isIdentifier(node.name) &&
        (TRANSFORM_PROPS as any).includes(node.name.text);
    }
