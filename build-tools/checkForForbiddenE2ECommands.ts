...
exec('git diff --name-only --cached', (error, stdout, stderr) => {
    ...
    files.forEach((file) => {
        readFile(file, 'utf8', (_err: NodeJS.ErrnoException, data: string) => {
            ...
            if (data.indexOf('xdescribe(') !== -1) {
                console.error('[checkForFdescribe] ERROR: Unexpected "xdescribe" in', file);
                result = true;
            }

            if (data.indexOf('fit(') !== -1) {
                console.error('[checkForFdescribe] ERROR: Unexpected "fit" in', file);
                result = true;
            }
